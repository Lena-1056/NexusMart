...
_(Note: If you'd like to view the architectural workflow or database Entity Relationship (ER) diagrams, you can find them in the `architecture.mmd` and `database_schema.mmd` files in the root directory)._
...
3. In the backend configuration, enter your DB password for the following services:
   - Auth service
   - Cart service
   - Payment service
   - Order service
...
_(Note: If you get an execution policy error, run `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` first)._
